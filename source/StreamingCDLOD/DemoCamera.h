...
protected:
   ...
   //
   bool                 m_bInitialSettingsSerialised;
   //
   uint32               m_observerID;
   //
   CDLODStreamingStorage* m_storage;
   //
   DemoCameraRecorder * m_recorder;
   //
   float                m_accumMouseDeltaX;
   float                m_accumMouseDeltaY;
   D3DXVECTOR3          m_accumMove;
...
public:
   ...
   //
public:
   void                 Initialize( CDLODStreamingStorage * storage, float minViewRange, float maxViewRange );
   //
   ...
   //
   float                GetInputSmoothingLerpK() const         { return m_inputSmoothingLerpK; }
   void                 SetInputSmoothingLerpK(float val)      { m_inputSmoothingLerpK = val; }
   //
   void                 SetPosition( D3DXVECTOR3 & position )  { m_vPos = position; }
   void                 SetOrientation( D3DXVECTOR3 & direction );
