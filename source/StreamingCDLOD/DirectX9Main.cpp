...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow )
{
   ...
   //MessageBox(NULL, lpCmdLine, L"command line", MB_OK );
   //return 0;


   if( !vaPreInitialize( vaStringSimpleNarrow( wstring(lpCmdLine) ).c_str() ) )
   {
      return 0;
   }
   ...
   //if( FAILED( DXUTCreateDevice( true, 640, 480 ) ) )
   //if( FAILED( DXUTCreateDevice( false, 640, 480 ) ) )
   //if( FAILED( DXUTCreateDevice( false, 1024, 768 ) ) )
   //if( FAILED( DXUTCreateDevice( false, 1680, 1050 ) ) )
   //if( FAILED( DXUTCreateDevice( false, 1920, 1200 ) ) )
   {
...
void CALLBACK OnD3D9FrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext )
{
   ...
   V( pd3dDevice->Clear( 0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB( 0, 100, 100, 128 ), 1.0f, 0 ) ); //D3DCOLOR_ARGB( 0, 45, 50, 170 )
