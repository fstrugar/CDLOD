...
//
void DemoSky::Initialize( )
{
   const IniParser & iniParser = vaGetIniParser();

   m_sunAzimuth = iniParser.getFloat("Sun:Azimuth", 200.0f / 180.0f * (float)PI);
   m_sunElevation = iniParser.getFloat("Sun:Elevation", 30.0f / 180.0f * (float)PI);
